        ...
        <NexusCard title="Récapitulatif" padding="lg" className="sticky top-12 border-none shadow-2xl ring-1 ring-slate-200/50">
          ...
          <div className="space-y-4">
            ...
            <div className="pt-4 border-t border-slate-100">
              <NexusButton variant="outline" className="w-full py-6 text-sm font-black uppercase tracking-widest border-2 border-indigo-600 text-indigo-600 hover:bg-indigo-600 hover:text-white h-20 transition-all" onClick={() => { setIsRequestingQuote(true); setQuoteResult(null); setTimeout(() => { const hasBulkDiscount = cart.length >= 2; const total = cart.reduce((acc, item) => acc + item.price, 0); const discountAmount = hasBulkDiscount ? total * 0.1 : 0; const finalTotal = total - discountAmount; const newQuote = { id: `QT-2024-00${quotes.length + 1}`, date: new Date().toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' }), total: finalTotal, status: 'Sent', probability: 85 + Math.floor(Math.random() * 10) }; setQuotes([newQuote, ...quotes]); setQuoteResult({ discount: hasBulkDiscount ? 10 : 0, terms: 'Net 30', pdfSent: true, probability: newQuote.probability }); setIsRequestingQuote(false); }, 3000); }}>Request Formal Quote</NexusButton>
            </div>
          </div>
        </NexusCard>
      </div>
